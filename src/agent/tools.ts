...
async function dockerRun(language: 'python' | 'node', code: string) {
  const host = path.resolve(process.cwd(), 'sandbox')
  const work = path.join(host, 'workspace')

  await fs.mkdir(work, { recursive: true })
  await fs.mkdir(path.join(host, 'uploads'), { recursive: true })
  await fs.mkdir(path.join(host, 'outputs'), { recursive: true })

  const ext = language === 'python' ? 'py' : 'js'
  const name = `.agent-run-${Date.now()}-${Math.random().toString(16).slice(2)}.${ext}`
  const local = path.join(work, name)

  await fs.writeFile(local, code, 'utf8')

  const image = process.env.SANDBOX_IMAGE ?? 'local-agent-sandbox'
  const memLimit = process.env.SANDBOX_MEMORY_LIMIT ?? '1024m'

  try {
    const command =
      language === 'python'
        ? ['python3', `/sandbox/workspace/${name}`]
        : ['node', `/sandbox/workspace/${name}`]
    ...
    const { stdout, stderr } = await exec(
      ...
      [
        ...
        '/tmp',
        '-v',
        `${host}/workspace:/sandbox/workspace`,
        '-v',
        `${host}/outputs:/sandbox/outputs`,
        '-v',
        `${host}/uploads:/sandbox/uploads:ro`,
        '--user',
        '1000:1000',
        image,
        ...command,
    ...
    return {
      ...
      error: 'Sandbox unavailable or code failed',
    }
  } finally {
...
export const handlers: Record<string, (args: any) => Promise<any>> = {
  ...
  'web.searchSearxng': async (a) => {
    const url = process.env.SEARXNG_URL ?? 'http://localhost:8080'
    try {
      const { data } = await axios.get(`${url}/search`, {
        params: { q: a.query, format: 'json' },
        timeout: 10000,
      })
      return { provider: 'searxng', results: data.results?.slice(0, 8) ?? data }
    } catch (error: any) {
      if (!process.env.TAVILY_API_KEY) {
        return {
          error: `SearXNG unavailable: ${error.message}`,
          fallback: 'TAVILY_API_KEY is not configured',
        }
      }
      const { data } = await axios.post(
        'https://api.tavily.com/search',
        { api_key: process.env.TAVILY_API_KEY, query: a.query, max_results: 5 },
        { timeout: 10000 }
      )
      return { provider: 'tavily-fallback', results: data }
    }
  },
