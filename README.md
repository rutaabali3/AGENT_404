...
The agent uses progressive disclosure. `rules.md` is always loaded, the catalog metadata is available to the model for every chat, relevant skill bodies are selected from the user request, and the skill bound to a dispatched tool is attached to that tool result. Imported skills that describe Manus-hosted capabilities are rewritten with an AGENT-420 compatibility section; the runtime compatibility notice requires the model to use only tools actually exposed by AGENT-420 and to report unavailable providers honestly. Bundled `references/`, `scripts/`, and `templates/` are stored for future use but are not executed automatically.

The original local skills are now detailed packages rather than one-line tool notes. `docx`, `pdf`, `xlsx`, `pptx`, `code-execution`, `filesystem`, `web-search`, `image-generation`, `vision`, `video-download`, and `ahm7-wrapper` each include expanded workflows plus reference files with authoritative documentation links and deeper implementation guidance. The duplicate Manus-only presentation package was removed; presentation requests route to the local `pptx` skill and `generate_pptx` handler.
